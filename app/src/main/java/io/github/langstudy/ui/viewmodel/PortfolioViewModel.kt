...
import io.github.langstudy.data.repository.PortfolioRepository
...
class PortfolioViewModel(
    ...
    val filteredItems: StateFlow<List<PortfolioItem>> =
        combine(_items, _searchQuery, _currentLanguage) { items, query, lang ->
            items.filter {
                (query.isBlank() || it.title.contains(query, ignoreCase = true) || it.link.contains(
                    query,
                    ignoreCase = true
                )) &&
